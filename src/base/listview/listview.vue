  ...
  @scroll="scroll" :probeType="probeType">
    ...
    <div class="list-shortcut"
      ...
      <ul>
        ...
        :class="{current: currentIndex === index}" :key="item.title"
        :data-index="index">
          {{item}}
        </li>
      </ul>
...
export default {
  ...
  watch: {
    ...
    scrollY(newY) {
      const listHeigth = this.listHeigth
      // 当滚动到顶部， newY > 0
      if (newY > 0) {
        this.currentIndex = 0
        return
      ...
      // 在中间部分滚动
      for (let i = 0; i < listHeigth.length - 1; i++) {
        let height1 = listHeigth[i]
        let height2 = listHeigth[i + 1]
      ...
      this.currentIndex = listHeigth.length - 2
    }
  }
}
</script>
